...
const MessageItem: React.FC<{ 
  ...
  return (
    <div className={`flex gap-3 mb-6 ${isMatch ? 'scale-[1.02] transform' : ''}`}>
      <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs text-white shrink-0 mt-1 ${getAvatarColor(senderName)}`}>
        {senderName[0]}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{senderName}</span>
          ...
          {isMatch && 'matched_phrases' in msg && (
            <div className="mt-2 flex flex-wrap gap-1">
              {(msg as ChatRecord).matched_phrases.map((phrase, idx) => (
                <span key={idx} className="bg-white/20 text-[10px] px-1.5 py-0.5 rounded flex items-center gap-1">
                  <ICONS.Tag />
                  {phrase}
                </span>
              ))}
...
export const ChatHistory: React.FC<ChatHistoryProps> = ({ record }) => {
  return (
    <div className="bg-slate-50/50 dark:bg-slate-900/50 rounded-2xl p-6 border border-slate-100 dark:border-slate-700">
      <div className="space-y-2 mb-8 text-center">
        <div className="inline-block px-3 py-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-full text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="relative my-10">
          ...
          <div className="relative flex justify-center">
            <span className="bg-indigo-100 dark:bg-indigo-900 text-indigo-600 dark:text-indigo-300 px-4 py-1 rounded-full text-xs font-bold uppercase tracking-tighter">
              Matched Content
            </span>
          </div>
        </div>
        ...
        <div className="relative my-10">
          ...
          <div className="relative flex justify-center">
            <span className="bg-slate-100 dark:bg-slate-700 text-slate-400 dark:text-slate-400 px-4 py-1 rounded-full text-xs font-bold uppercase tracking-tighter">
              Following Context
            </span>
          </div>
        </div>
